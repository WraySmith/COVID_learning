...
def add_series(df, columns_analyze):
    for column in df:
        if column in columns_analyze:
            ...
            df[column + "_2wknormal"] = (df[column] - df[column + "_mean"]) / df[
                column + "_stdev"
            ]
            # create a column for difference of 2 week normalized column
            df[column + "_2wknormal_diff"] = df[column + "_2wknormal"] - df[
                column + "_2wknormal"
            ].shift(1)
...
def explore_series(df, columns_explore, plot_lags=20):
    ...
    for column in df:
        if column in columns_explore:
            # time history plot
            df[column].plot(
                ax=plt.subplot2grid((5, plot_cols), (0, position)), title=column
            )
            # histogram
            df[column].hist(ax=plt.subplot2grid((5, plot_cols), (1, position)))
            # qqplot to check normality
            qqplot(
                df[column], line="r", ax=plt.subplot2grid((5, plot_cols), (2, position))
            )
...
def evaluate_arima_model(
    train, test, arima_order, label="", predict_steps=1, confidence=False
):
    name = []
    step_name = [
        ("arima_predict_" + label + "_step" + str(i))
        for i in np.arange(1, (predict_steps + 1))
    ]
    ...
    for t in range(len(test)):
        if confidence == True:
            yhat, ylow, yhigh = arima_model(
                history, arima_order, predict_steps, confidence
            )
        else:
...
def grid_ARIMA_models(train, test, p_values, d_values, q_values):
    ...
    output = pd.DataFrame(
        list(zip(p_list, d_list, q_list, rmse_list)), columns=["p", "d", "q", "rmse"]
    )
    return output, best
...
def pdq_heat(df):
    ...
    for d in d_list:
        ...
        sns.heatmap(
            temp_df, annot=True, ax=plt.subplot2grid((1, plot_cols), (0, position))
        )
        position += 1
    return fig_out
...
def evaluate_models(splits, df, data_col, arima_order):
    ...
    for train_index, test_index in splits.split(df[data_col]):
        ...
        out_data = pd.merge(
            out_data, temp_results, left_index=True, right_index=True, how="outer"
        )
        ...
        try:
            temp_results, temp_rmse = evaluate_arima_model(
                train, test, arima_order, label=str(idx), predict_steps=1
            )
            out_data = pd.merge(
                out_data, temp_results, left_index=True, right_index=True, how="outer"
            )
    ...
    out_rmse = pd.DataFrame(
        columns=col_labels, index=["Persistence", "Roller", "ARIMA"]
    )
...
def multistep_forecast(
    splits, df, data_col, arima_order, forecast_steps, confidence=False
):
    idx = 0
    out_data = df.copy()
    ...
    for train_index, test_index in splits.split(df[data_col]):
        ...
        if confidence == False:
            try:
                temp_results, _ = evaluate_arima_model(
                    train,
                    test,
                    arima_order,
                    label=str(idx),
                    predict_steps=forecast_steps,
                    confidence=False,
                )
            except:
                continue
        # include low and high confidence limits if required
            ...
            try:
                temp_results, _, low_temp, high_temp = evaluate_arima_model(
                    train,
                    test,
                    arima_order,
                    label=str(idx),
        ...
        for i in np.arange(forecast_steps):
            ...
            if confidence == True:
                out_low_data.iloc[:, start + i] = out_low_data.iloc[:, start + i].shift(
                    i
                )
                out_high_data.iloc[:, start + i] = out_high_data.iloc[
                    :, start + i
                ].shift(i)
            # create the temp df for rmse comparison and append calculated rmse
            temp_df = pd.concat(
                [out_data.iloc[:, start + i], out_data.loc[:, "daily_case"]], axis=1
            )
